...
export default function InventoryPage() {
  ...
  return <><PageHeader title="Tồn kho" description="Số lượng theo từng kho và SKU; tồn khả dụng được tính từ quantity - reservedQuantity." />
    ...
    {!loading && !error && (data?.content?.length ? <section className="panel inventory-table-wrap"><div className="data-section-heading"><div><h2>Danh sách tồn kho</h2><p>{data.totalElements} dòng theo kho và SKU</p></div></div><table className="product-table"><thead><tr><th>Kho</th><th>SKU</th><th>Sản phẩm</th><th>Tracking</th><th>Tồn thực</th><th>Đã giữ</th><th>Khả dụng</th></tr></thead><tbody>{data.content.map(row => <tr key={row.inventoryId}><td>{row.warehouseName}</td><th scope="row">{row.sku}</th><td>{row.productName}</td><td>{row.trackingType}</td><td>{row.quantity}</td><td>{row.reservedQuantity}</td><td><strong className={row.availableQuantity > 0 ? 'quantity-positive' : 'quantity-zero'}>{row.availableQuantity}</strong></td></tr>)}</tbody></table></section> : <section className="panel empty-state"><p>Không có dữ liệu tồn kho phù hợp.</p></section>)}
