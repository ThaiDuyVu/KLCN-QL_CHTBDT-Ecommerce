...
export default function SerialsPage() {
  ...
  return <><PageHeader title="Serial / IMEI" description="Tra cứu thiết bị vật lý theo serial, IMEI, SKU, kho và trạng thái." /><nav className="inventory-tabs"><Link to="/inventory">Tồn kho</Link><Link to="/goods-receipts">Phiếu nhập</Link><Link className="active" to="/serials">Serial / IMEI</Link></nav>
    ...
    {loading && <p role="status">Đang tải serial…</p>}{error && <p className="auth-alert" role="alert">{error}</p>}{!loading && !error && (data?.content?.length ? <section className="panel inventory-table-wrap"><div className="data-section-heading"><div><h2>Danh sách thiết bị</h2><p>{data.totalElements} Serial/IMEI</p></div></div><table className="product-table"><thead><tr><th>Serial</th><th>IMEI</th><th>SKU</th><th>Kho</th><th>Trạng thái</th><th></th></tr></thead><tbody>{data.content.map(row => <tr key={row.serialId}><th>{row.serialNumber}</th><td>{row.imeis.map(i => i.imeiNumber).join(', ') || '—'}</td><td>{row.sku}</td><td>{row.warehouseName}</td><td><StatusBadge status={row.status}>{row.status}</StatusBadge></td><td><Link className="button button-quiet" to={`/serials/${row.serialId}`}>Chi tiết</Link></td></tr>)}</tbody></table></section> : <section className="panel empty-state"><p>Không có serial/IMEI phù hợp.</p></section>)}
