...
export default function AddToCart({ variantId, compact = false }) {
  ...
  return <form onSubmit={submit} className={`commerce-inline${compact ? ' commerce-inline-compact' : ''}`}>
    {!compact && <label>Số lượng<input type="number" min="1" max="2147483647" step="1" value={quantity} disabled={busy} onChange={(e) => setQuantity(e.target.value)} required /></label>}
    <button className="button" disabled={busy}>{busy ? 'Đang thêm…' : 'Thêm vào giỏ'}</button>
