...
export default function WarrantyTicketsPage({ customer = false }) {
  ...
  return <><PageHeader title={customer ? 'Yêu cầu bảo hành của tôi' : 'Warranty Ticket'} description="Theo dõi lịch sử tiếp nhận và xử lý bảo hành." /><div className="warranty-links"><Link className="button button-quiet" to={customer ? '/my-warranties' : '/warranties'}>{customer ? 'Bảo hành của tôi' : 'Tra cứu bảo hành'}</Link></div>
    ...
    {loading && <p role="status">Đang tải ticket…</p>}{error && <p className="auth-alert" role="alert">{error}</p>}{!loading && !error && (data?.content?.length ? <section className="panel warranty-table-wrap"><div className="data-section-heading"><div><h2>Danh sách yêu cầu</h2><p>{data.totalElements} yêu cầu bảo hành</p></div></div><table className="product-table"><thead><tr><th>Ticket</th><th>Thiết bị</th><th>Serial</th><th>Khách hàng</th><th>Trạng thái</th><th>Ngày tạo</th><th></th></tr></thead><tbody>{data.content.map(row => <tr key={row.ticketId}><th>{row.ticketCode}</th><td>{row.productName}<br /><span className="muted">{row.sku}</span></td><td>{row.serialNumber}</td><td>{row.customerName}</td><td><StatusBadge status={row.status}>{ticketStatus[row.status]}</StatusBadge></td><td>{formatDate(row.createdAt)}</td><td><Link className="button button-quiet" to={customer ? `/my-warranty-tickets/${row.ticketId}` : `/warranty-tickets/${row.ticketId}`}>Chi tiết</Link></td></tr>)}</tbody></table></section> : <section className="panel empty-state"><p>Chưa có Warranty Ticket.</p></section>)}
