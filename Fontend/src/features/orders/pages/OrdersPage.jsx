  ...
  return <><PageHeader title={customer ? 'Đơn hàng của tôi' : 'Quản lý đơn hàng'} /><CommerceState loading={isLoading} error={error} retry={retry} />
    {!isLoading && !error && (data?.content?.length ? <section className="panel commerce-section">
      ...
      <div className="commerce-scroll"><table className="product-table"><thead><tr><th>Mã đơn</th><th>Ngày đặt</th><th>Chi nhánh</th><th>Người nhận</th><th>Trạng thái</th><th>Tổng tiền</th><th>Thao tác</th></tr></thead><tbody>
        {data.content.map((order) => <tr key={order.orderId}><th scope="row">{order.orderCode}</th><td>{date(order.orderDate)}</td><td>{order.warehouseName || '—'}</td><td>{order.recipientName}</td><td>{statusLabels[order.status]}</td><td>{money(order.totalAmount)}</td><td><Link className="button button-quiet" to={`${base}/${order.orderId}`}>Chi tiết</Link></td></tr>)}
      </tbody></table></div>
